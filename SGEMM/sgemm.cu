...
template<
        ...
        const bool ENBALE_DOUBLE_BUFFER //数据预处理
        >
__global__ void Sgemm(float * __restrict__ A,
    ...
    const int K)
{
    int bx = blockDim.x;  int by = blockDim.y; //block index
    int tx = threadIdx.x; int ty = threadIdx.y; //thread index

    const int THREAD_X_PER_BLOCK = BLOCK_SIZE_N / THREAD_SIZE_X; // bn/rn
    const int THREAD_Y_PER_BLOCK = BLOCK_SIZE_M / THREAD_SIZE_Y;
    const int THREAD_NUM_PER_BLOCK = THREAD_X_PER_BLOCK * THREAD_Y_PER_BLOCK;
    
    const int tid = ty * THREAD_X_PER_BLOCK + tx;//线程在block块内的索引
    ...
    //thread number in a row
    const int A_TILE_THREAD_PER_ROW = BLOCK_SIZE_K / 4;
    const int B_TILE_THREAD_PER_ROW = BLOCK_SIZE_N / 4; 
    
    //row number and col number that needs to be loaded by this thread
    const int A_TILE_ROW_START = THREAD_NUM_PER_BLOCK / A_TILE_THREAD_PER_ROW;
    const int B_TILE_ROW_START = THREAD_NUM_PER_BLOCK / B_TILE_THREAD_PER_ROW;
...
#pragma unroll
    for(int i=0;i<BLOCK_SIZE_M;i+=A_TILE_ROW_STRIDE) {
        ...
        FETCH_FLOAT4(ldg_a_reg[ldg_index]) = FETCH_FLOAT4(A[OFFSET(A_TILE_ROW_START + i,A_TILE_COL,K)]);
        As[0][A_TILE_COL][A_TILE_ROW_START+i] = ldg_a_reg[ldg_index];
        As[1][A_TILE_COL+1][A_TILE_ROW_START+i] = ldg_a_reg[ldg_index+1];
        As[2][A_TILE_COL+2][A_TILE_ROW_START+i] = ldg_a_reg[ldg_index+2];
        As[3][A_TILE_COL+3][A_TILE_ROW_START+i] = ldg_a_reg[ldg_index+3];
    }
...
#pragma unroll
    ...
    do{
        ...
        if(tile_idx < K) {
            ...
            #pragma unroll
            for(int i=0;i<BLOCK_SIZE_K;i+=B_TILE_ROW_START) {
                int ldg_index = i / B_TILE_ROW_STRIDE * 4;
                FETCH_FLOAT4(ldg_b_reg[ldg_index]) = FETCH_FLOAT4(B[OFFSET(B_TILE_ROW_START+i,B_TILE_COL+tile_idx,N)]);
        ...
        for(int j=0;j<BLOCK_SIZE_K-1;++j) {
            //load next tile from sharedMem to register
            #pragma unroll 
            for(int thread_y=0;thread_y<THREAD_SIZE_Y;thread_y+=4) {
        ...
        //load A and B from shared mem to register
        #pragma unroll
        for(int thread_y=0;thread_y<THREAD_SIZE_Y;thread_y+=4) {
    ...
    for(int thread_y=0;thread_y<THREAD_SIZE_Y;++thread_y) {
        ...
        for(int thread_x=0;thread_x<THREAD_SIZE_X;thread_x+=4) {
            FETCH_FLOAT4(C[OFFSET(
                BLOCK_SIZE_M * by + ty * THREAD_SIZE_X + thread_y,
                BLOCK_SIZE_N * bx + tx * THREAD_SIZE_Y + thread_x,
                N
...
int main(int argc,char** argv) 
{
    ...
    double flopPerMatrixMul = 2.0 * M * K * N;
    ...
    gigaFlops[1] = (flopPerMatrixMul * 1.0e-9f) / (msecPerMatrixMul[1] / 1000.0f);
