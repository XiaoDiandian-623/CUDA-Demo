...
//该命名空间存储 有关fp32/fp16 fma和add的各种重载操作
namespace gemv2 {
    struct half8 {
        half2 h1;
        half2 h2;
    ...
    {   
        ...
        // 有的编译器会不认识half intrinsic 例如__hmul或者__hadd，这很奇怪 所以粗暴转成fp32计算再转回fp16
...
// THREADS_PER_VALUE 每行需要多少线程处理
// VEC_SIZE FP16:8 FP32:4
...
__global__ void gemv2_kernel(float* matrix, float* vector, float* res, int N, int M) {
    ...
    int mat_o = tid / THREADS_PER_VALUE; // 每个线程负责数据所在行号
    int mat_i = tid % THREADS_PER_VALUE * VEC_SIZE; // 每个线程负责数据所在向量号
    
    constexpr int ROW_PER_ITER = THREADS_PER_BLOCK / THREADS_PER_VALUE;// 一个block处理的行数 作步长使用
    __shared__ float out_smem[512];

    float4 out;
    // 点乘或fma(fused mul add)，inter-block循环累加
    for (int ti = mat_o; ti < N; ti += ROW_PER_ITER) {
        float4 mat = *reinterpret_cast<float4*>(&matrix[ti * M + mat_i]);
        float logits = vector[ti];
        
        out = gemv2::fma(logits, mat, out);// fused mul and add: d = a * b + c
    }
    // intra-block 二分法相加得最终结果
...
// TODO: 如何修改float4部分为可以泛化表示float4和half8类型的代码, 而后此模板函数可以取代以上fp32和fp16的gemv2
