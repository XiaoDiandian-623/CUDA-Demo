...
__global__ void __launch_bounds__((BM * BN) / (TM * TN), 1) sgemm_blocktiling_2d_kernel(float *A, float *B, float *C, int M, int N, int K)
{
    ...
    float thread_results[TM * TN] = {0.0};//每个线程需要计算TM*TN大小的矩阵
    float reg_m[TM] = {0.0};//A的一行
    float reg_n[TN] = {0.0};//B的一列
    ...
    for (uint bk_idx = 0; bk_idx < K; bk_idx += BK)
    {
        ...
        for (uint dot_idx = 0; dot_idx < BK; ++dot_idx)
        {
            ...
            for (uint reg_idx_m = 0; reg_idx_m < TM; ++reg_idx_m)
            {
                ...
                {
                    thread_results[reg_idx_m * TN + reg_idx_n] += reg_m[reg_idx_m] * reg_n[reg_idx_n];
                }
            }
        }
        __syncthreads();
    ...
    for (uint reg_idx_m = 0; reg_idx_m < TM; ++reg_idx_m)
    {
        for (uint reg_idx_n = 0; reg_idx_n < TN; ++reg_idx_n)
        {
            C[(thread_row * TM + reg_idx_m) * N + thread_col * TN + reg_idx_n] = thread_results[reg_idx_m * TN + reg_idx_n];
        }
    }
}
