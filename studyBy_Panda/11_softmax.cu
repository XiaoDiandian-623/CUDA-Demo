...
//模板实例化 warp_reduce : ReductionOp(max or sum )
...
__global__ void WarpSoftmax(const float* src, float* dst, const int rows, const int cols) {
  ...
  const int step = num_global_warp * rows_per_thread; // 1000 一个warp可能处理多行
  // 进入到当前所分配的整个block数量的数值处理范围
  for (int row = global_warp_id * rows_per_thread; row < rows; row += step) {
    float thread_max[rows_per_thread];
    // 细粒度化，进入到每个线程所处理的行数范围---处理这一行的数据 最大值
    for (int row_id = 0; row_id < rows_per_thread; ++row_id) {
      thread_max[row_id] = -Inf<float>();
      float* row_buf = buf[row_id];
      // 再细粒度一点，进入到每个线程所处理的一行的多个向量范围 ---处理这个向量范围内的数据
      for (int pack_id = 0; pack_id < num_packs; ++pack_id) {
       
        const int pack_offset = pack_id * pack_size; // 每个向量的起始偏移
        const int col = (pack_id * warp_width + lane_id) * pack_size; // 当前向量所在的起始列号
        if (col < cols) {
          ...
          for (int i = 0; i < pack_size; ++i) { row_buf[pack_offset + i] = -Inf<float>(); }  //起始列号超出总列数，则设为负无穷，对softmax值无影响
        }
      }
    }
   
    float warp_max[rows_per_thread]; // 声明rows_per_thread个寄存器保存当前线程计算的行的最大值
    for (int row_id = 0; row_id < rows_per_thread; ++row_id) { // reduce各个线程计算的最大值，得出所有线程中的最大值，即一行的最大值
...
int main(){
    ...
    const int N = 1000 * 1024; // 1000行 每行1024个数据
